...
GPIO_ErrorStatus_t GPIO_InitPin(GPIO_Pin_Cfg_t * Ptr_PinObject)
{
    ...
    uint32 Local_MODER= ((GPIO_PORT_t  *) (Ptr_PinObject->Port))    -> MODER;
    uint32 Local_OTYPER= ((GPIO_PORT_t *) (Ptr_PinObject->Port))    -> OTYPER;
    uint32 Local_PUPDR = ((GPIO_PORT_t *)  (Ptr_PinObject->Port))   -> PUPDR;
    uint32 Local_OSPEEDR=((GPIO_PORT_t *)  (Ptr_PinObject -> Port)) ->OSPEEDR;
   ...
   //Assign values in the Registers
    ((GPIO_PORT_t *)  (Ptr_PinObject->Port))    -> MODER  =  Local_MODER;
    ((GPIO_PORT_t *)  (Ptr_PinObject->Port))    -> OTYPER =  Local_OTYPER ;
    ((GPIO_PORT_t *)  (Ptr_PinObject->Port))    -> PUPDR  =  Local_PUPDR  ;
    ((GPIO_PORT_t *)  (Ptr_PinObject->Port))    -> OSPEEDR= Local_OSPEEDR;
...
GPIO_ErrorStatus_t GPIO_SetPinValue ( void * Port ,uint32 Pin ,uint32 Value)
{
    ...
    case GPIO_OUTPUT_HIGH :
    	                      ((GPIO_PORT_t *)  Port) ->BSRR = (ONE_BIT_MASK<< Pin);
    	                       Local_ReturnErrorState=GPIO_OK; break;

    case GPIO_OUTPUT_LOW:
    	                      ((GPIO_PORT_t *)  Port) ->BSRR = (ONE_BIT_MASK<< (Pin+ PORT_SIZE ));
    	                        Local_ReturnErrorState=GPIO_OK;  break;
...
GPIO_ErrorStatus_t GPIO_GetPinValue ( void * Port ,uint32 Pin ,uint8 * Get_Value)
{
    ...
    *Get_Value =(  ( ( (GPIO_PORT_t *)  Port ) ->IDR >> Pin )& ONE_BIT_MASK  );
	return Local_ReturnErrorState;
